...
class MyWorkoutsScreen(Screen):
    ...
    def delete_workout(self, workout_name):
        def do_delete(instance):
            ...
            self.on_pre_enter()
            popup.dismiss()
        layout = BoxLayout(orientation='vertical', spacing=14, padding=[14,14,14,14])
        layout.add_widget(Label(text=f"Delete [b]{display_name(workout_name)}[/b]?", markup=True, color=(1,1,1,1)))
        btns = BoxLayout(orientation='horizontal', size_hint_y=None, height=44, spacing=12)
        btns.add_widget(Button(text="Cancel", on_release=lambda i: popup.dismiss()))
        btns.add_widget(Button(text="Delete", background_color=(0.9,0.2,0.2,1), color=(1,1,1,1), on_release=do_delete))
